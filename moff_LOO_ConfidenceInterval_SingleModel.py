...
def train_models_3(data_A,data_B):
    ...
    size_train= int (data_A.shape[0] * 0.4)
    ...
    kern = GPy.kern.RBF(input_dim=1, lengthscale=0.25, variance=0.03)
    k_bias= GPy.kern.Bias(1,variance=1.5)
...
def run_mbr(args):
    ...
    #for list_inter in [250]:
        for pep_out in intersect_share[0:50] :
            print '-- ', pep_out ,'-- '
           ...
           # print 'input columns',pre_pep_save[0].shape
            ...
            if n_replicates == 2:
                ...
                #'mz',
                test = reduce(
                    lambda left, right: pd.merge(left, right, on=['code_unique','peptide',  'charge', 'prot'], how='outer'),
                    pre_pep_save)
           ...
           #                             model_err, args.w_comb,interval, 1,(l_int /2) ),axis=1)
            ...
            #test['time_base'] = test.ix[:, 5 : ].apply( lambda x: combine_model(x, model_save,model_err, args.w_comb ,log_mbr) ,axis=1)

            # GP computation
            test['time_base']= test[['rt_0','rt_1','rt_2','rt_3','rt_4','rt_5','rt_6','rt_7','rt_8','rt_9']].apply(  lambda x: combine_model_GP(x, model_save,model_err, args.w_comb ,log_mbr) ,axis=1)
            #test['time_ev'] = test[['rt_0','rt_1','rt_2','rt_3','rt_4','rt_5','rt_6','rt_7','rt_8','rt_9']].apply(lambda x:combine_model_GP_consonant_bf(x, model_save, model_err, args.w_comb, log_mbr, interval, 0.5,(l_int /2)) ,axis=1)
